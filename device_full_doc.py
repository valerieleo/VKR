...
# интревал между циклами обработки
if len(sys.argv) == 2 :    
...
def read(filename):
    ...
    print ('center:',x,y)
    cv2.circle(imgg, (x, y), r, (0, 255, 255), 3, cv2.LINE_AA) 
    cv2.circle(imgg, (x, y), 2, (0, 255, 255), 3, cv2.LINE_AA)
    cv2.imwrite('results/circles.jpg', imgg)
    ...
    for i in range (temp.shape[0]):
        for j in range (temp.shape[1]):
            if (j-x)**2+(i-y)**2>=r**2:
                temp[i][j]=255
    cv2.imwrite('results/template.jpg', temp)
    ...
    cv2.imwrite('results/polar.jpg', img_lin)
    ...
    cv2.line(result, (0,u1), (size,u1), (0,0,255), 3)
    cv2.line(result, (0,u2), (size,u2), (0,255,255), 3)
    cv2.imwrite('results/proj_col.jpg', result)
